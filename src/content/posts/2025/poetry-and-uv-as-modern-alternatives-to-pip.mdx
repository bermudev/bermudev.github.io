...
- There is no strong dependency resolution (it depends on how we generate the `requirements.txt` file).
...
> [!danger] Dependency confusion
> This problem appears when a private dependency does not exist on PyPI under the same name. If someone publishes a public package with that name, `pip` may install it instead of the private package, depending on how `index-url` and `extra-index-url` are configured.
