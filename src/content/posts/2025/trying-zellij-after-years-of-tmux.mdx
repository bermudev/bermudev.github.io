...
> [!warning] This command overwrites the configuration
> The `>` redirection below replaces `config.kdl` if it already exists. Back up your current configuration before running it.
...
```kdl file="~/.config/zellij/config.kdl"
pane_frames true
mouse_mode true
